...
# # Allow user assigned identity to manage AKS items in kubernetes rg to adjust custom routes 
#  resource "azurerm_role_assignment" "aks_user_assigned_rg" {
#    principal_id         = azurerm_kubernetes_cluster.main.kubelet_identity[0].object_id
#    scope                = format("/subscriptions/%s/resourceGroups/%s", data.azurerm_subscription.main.subscription_id, data.azurerm_resource_group.main_2.name)
#    role_definition_name = "Contributor"
# }
